...
//	maps\_zombiemode_auto_turret::init();
	...
	//PERK_CUSTOM_WUNDERFIZZ_THREAD
	maps\_zombiemode_wunderfizz::init();
	//PACK_PUNCH_ORIGINS_THREAD
	maps\_zombiemode_pack_puch_origins::init();
	//PACK_PUNCH_DEFAULT_THREAD
	maps\_zombiemode_pack_puch::init();
	//CUSTOM_POWER_UPS_THREAD
	maps\_zombiemode_custom_powerups::init();
	//PACK_PUNCH_IW_THREAD
	maps\_zombiemode_iw_pack_puch::init();
	//MYSTERY_BOX_ORIGINS_THREAD
	maps\_zombiemode_origins_mystery_box::init();
	//MYSTERY_BOX_MAGIC_WHEEL_THREAD
	maps\_zombiemode_iw_magic_wheel::init();
	//MYSTERY_BOX_SHADOWS_THREAD
	maps\_zombiemode_shadows_mystery_box::init();
	/#
...
precache_models()
{
	...
	//IW_HUD_PRECACHE
	PrecacheShader( "IW_HUD_MAIN_1" );
 	PrecacheShader( "IW_HUD_MAIN_2" );
 	PrecacheShader( "IW_HUD_MAIN_3" );
 	PrecacheShader( "IW_HUD_MAIN_4" );
}
...
init_standard_zombie_anims()
{
	...
	level.scr_anim["zombie"]["sprint1"] = %AI_ZOMBIE_SPRINT_ANIM_1;
	level.scr_anim["zombie"]["sprint2"] = %AI_ZOMBIE_SPRINT_ANIM_2;
	level.scr_anim["zombie"]["sprint3"] = %AI_ZOMBIE_SPRINT_ANIM_3;
	level.scr_anim["zombie"]["sprint4"] = %AI_ZOMBIE_SPRINT_ANIM_4;
...
chalk_one_up()
{
	...
	huds[1] SetText( "SCENE: " + level.round_number );
	...
	huds[1] FadeOverTime( 0.5 );
	huds[1].alpha = 0;

	wait( 0.5 );

	huds[1] FadeOverTime( 2 );
	huds[1].alpha = 1;
	huds[1].color = ( 0.8, 0.8, 0.8 );

	wait( 2 ); 

	huds[1] FadeOverTime( 1 );
	huds[1].color = ( 1, 1, 1 );
...
player_damage_override( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, modelIndex, psOffsetTime )
{
	...
	//PERK_CUSTOM_TOMBSTONE_THREAD
	self.invetory_backup = self maps\_zombiemode_perks_custom::player_set_inventory_backup();
...
end_game()
{
	...
	for( i = 0; i < players.size; i++ )
	{
		...
		if( IsDefined(level.end_game_round) && level.end_game_round )
		{
			game_over[i] SetText( "END_GAME_TEXT" );
		}
		else
		{
