...
zombie_spawn_init( animname_set )
{
	...
	self.gibbed = ZOMBIE_CRAWLERS_ENABLE; //Default: false
...
//	self random_tan(); 
	...
	//CUSTOM_POWER_UPS_ZOMBIE
	if( !self.no_powerups && !maps\shippuden_utility::Is_Boss( self ) )
	{
		if( maps\_zombiemode_custom_powerups::zombie_can_powerup() )
		{
			self.no_powerups = true;
			self thread maps\_zombiemode_custom_powerups::powerup_drop();
		}
	}
...
//	self thread print_run_speed( rand );
	if( rand <= 35 )
	{
		self.zombie_move_speed = "ZOMBIE_SPEED_RAND_1"; 
	}
	else if( rand <= 70 )
	{
		self.zombie_move_speed = "ZOMBIE_SPEED_RAND_2"; 
	}
	else
	{	
		self.zombie_move_speed = "ZOMBIE_SPEED_RAND_3"; 
	}
}
...
//	self endon( "death" ); 
	...
	while( 1 )
	{
		...
		//CANDY_ZOMBIE_HIT_THREAD
		self thread maps\_zombiemode_iw_candy_perks::perks_zombie_hit_effect(amount, attacker, point, type);
...
zombie_damage( mod, hit_location, hit_origin, player, amount )
{
	...
	//ZOMBIE_SPAWNER_HITMARKER_DAMAGE
...
zombie_damage_ads( mod, hit_location, hit_origin, player, amount )
{
	...
	//ZOMBIE_SPAWNER_HITMARKER_DAMAGE_ADS
	if( IsDefined( player ) && IsPlayer( player ) && mod != "MOD_BURNED" )
    {
		player thread maps\_damagefeedback::updateDamageFeedback( self );
    }
...
zombie_death_event( zombie )
{
	...
	//ZOMBIE_SPAWNER_HITMARKER_DEATH_EVENT
