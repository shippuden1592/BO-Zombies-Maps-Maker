...
solo_powerup_hud_overlay()
{
	...
	for( p = 0; p < players.size; p++ )
	{
		...
		for(i = 0; i < players[p].solo_powerup_hud_array.size; i++)
		{
			...
			players[p].solo_powerup_hud[i].y = players[p].solo_powerup_hud[i].y - 50; // ww: used to offset by - 78
...
start_carpenter( origin )
{
	...
	for(i = 0; i < players.size; i++)
	{
		...
		{
			players[i] maps\_zombiemode_score::player_add_points( "carpenter_powerup", 400 );
		}
		else
		{
